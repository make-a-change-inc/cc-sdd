...
> インストール手順だけ知りたい場合は [インストール](#-インストール) へジャンプ。v1.1.5 維持なら `npx @mac-inc/cc-sdd@1.1.5 --claude-code ...`、v2 移行は [Migration Guide](../../docs/guides/migration-guide.md) / [日本語版](../../docs/guides/ja/migration-guide.md) を参照。
...
npx @mac-inc/cc-sdd@latest --lang ja    # 日本語
npx @mac-inc/cc-sdd@latest --lang zh-TW # 繁体字中国語
npx @mac-inc/cc-sdd@latest --lang es    # スペイン語
...（対応言語: en, ja, zh-TW, zh, es, pt, de, fr, ru, it, ko, ar, el）

# エージェントオプション（デフォルト: claude-code / --claude）
npx @mac-inc/cc-sdd@latest --claude --lang ja        # Claude Code（11コマンド、対応言語は任意）
npx @mac-inc/cc-sdd@latest --claude-agent --lang ja  # Claude Code Subagents（12コマンド + 9サブエージェント）
npx @mac-inc/cc-sdd@latest --cursor --lang ja        # Cursor IDE
npx @mac-inc/cc-sdd@latest --gemini --lang ja        # Gemini CLI
npx @mac-inc/cc-sdd@latest --codex --lang ja         # Codex CLI
npx @mac-inc/cc-sdd@latest --copilot --lang ja       # GitHub Copilot
npx @mac-inc/cc-sdd@latest --qwen --lang ja          # Qwen Code
npx @mac-inc/cc-sdd@latest --opencode --lang ja      # OpenCode（11コマンド）
npx @mac-inc/cc-sdd@latest --opencode-agent --lang ja # OpenCode Subagents（12コマンド + 9サブエージェント）
...
**使用方法**: `npx @mac-inc/cc-sdd@latest --lang <コード>` (例: 日本語の場合は `--lang ja`)
...
```bash
# 言語とプラットフォーム
npx @mac-inc/cc-sdd@latest --lang ja            # macOS / Linux / Windows（自動検出）
npx @mac-inc/cc-sdd@latest --lang ja --os mac   # 旧来のフラグとして任意指定
